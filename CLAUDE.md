...
A standalone 1:1 direct-messaging app (phase 3 of a larger plan — see below). Any
registered user can self-signup and message another registered user, but only once
they're mutual friends via an add-by-ID request/accept flow; there are no roles,
groups, or admin overrides. The app is installable (PWA) and can send Web Push
notifications for messages that arrive while it isn't open. See `README.md` for
setup, the API endpoint table, and available scripts.
