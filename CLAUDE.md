...
A standalone 1:1 direct-messaging app (phase 2 of a larger plan — see below). Any
...
groups, or admin overrides. See `README.md` for setup, the API endpoint table, and
available scripts.
...
2. **PWA + Web Push.** A manifest, service worker, install-to-home-screen, and Web Push (VAPID) so a message can notify a user who has no live SSE connection open. Push should be the fallback for "not connected," not a duplicate of every SSE event. Friend-request events (new incoming request, a request accepted) have the same "no live push" gap today — `web/src/context/FriendsContext.jsx` only refetches on mount and after actions taken through it, so this phase should extend to those too, not just chat messages.
