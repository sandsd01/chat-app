...
const { pruneArchivedMessages } = require("../src/lib/drive");
...
describe("expiring messages and the Drive archive", () => {
  let alice;
  let bob;
  let conversation;
  ...
  // The regression test for the data-loss bug the design spec calls out:
  // prune trusts "every id at or below the watermark has been archived", and
  // an expiring message is deliberately never archived. Without prune
  // excluding them, this message would be deleted well before its timer.
  test("prune does not delete an unexpired expiring message below the watermark", async () => {
    const ephemeral = await sendAt(conversation.id, alice.id, "ephemeral", new Date(Date.now() + 600_000));
    const permanent = await sendAt(conversation.id, alice.id, "permanent", null);

    // Both participants have archived everything up to and including the
    // permanent message, which sits above the ephemeral one.
    for (const user of [alice, bob]) {
      await prisma.driveArchiveFile.create({
        data: {
          userId: user.id,
          conversationId: conversation.id,
          driveFileId: `file-${user.id}`,
          lastArchivedMessageId: permanent.id,
        },
      });
    }
    ...
    assert.ok(
      await prisma.message.findUnique({ where: { id: ephemeral.id } }),
      "an unexpired expiring message must survive prune"
    );
    assert.equal(await prisma.message.findUnique({ where: { id: permanent.id } }), null);
  });
});
